...
# the sorting function will fail later if it encounters the 1st file,
# which doesn't end with the underscore/integer
regex = re.compile('.*(rst)$')
matchlist = [matches.group(0) for line in rsts for matches in [regex.search(line)] if matches]
for m in matchlist:
    if m is not None:
        os.system('mv {0}.rst {0}.rst_0'.format(namein))
        rsts[rsts.index(m)] += '_0'
