...
options_socflags = list(itertools.product(range(1 + 1), range(4 + 1), range(4 + 1), range(1 + 1)))
...
def main(args):
    ...
    if args.all_rel_method or args.all:
        reldicts = add_flags_to_list_of_dicts("rel", "method", flags_rel_method, reldicts)
    if args.all_rel_modelpot or args.all:
        reldicts = add_flags_to_list_of_dicts("rel", "modelpot", flags_rel_modelpot, reldicts)
    if args.all_rel_modeldens or args.all:
        reldicts = add_flags_to_list_of_dicts("rel", "modeldens", flags_rel_modeldens, reldicts)
    if args.all_rel_dkh_order or args.all or args.all_dkh:
        reldicts = add_flags_to_list_of_dicts("rel", "order", flags_rel_order, reldicts)
    if args.all_rel_finitenuc or args.all or args.all_dkh:
        reldicts = add_flags_to_list_of_dicts("rel", "finitenuc", flags_rel_finitenuc, reldicts)
    if args.all_rel_picturechange or args.all:
        reldicts = add_flags_to_list_of_dicts(
            "rel", "picturechange", flags_rel_picturechange, reldicts
        )
    if args.all_rel_dkh_fpfwtrafo or args.all or args.all_dkh:
        reldicts = add_flags_to_list_of_dicts("rel", "fpfwtrafo", flags_rel_fpfwtrafo, reldicts)
    ...
    if args.all_rel_socflags_curated or args.all:
        ...
        reldicts = add_flags_to_list_of_dicts("rel", "socflags", comma_sep_flags, reldicts)
    elif args.all_rel_socflags:
        comma_sep_flags = [make_comma_sep_flags(flags) for flags in options_socflags]
        reldicts = add_flags_to_list_of_dicts("rel", "socflags", comma_sep_flags, reldicts)
    else:
        pass
