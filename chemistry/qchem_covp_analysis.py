...
from vmd_templates import pad_left_zeros, vmd_covp_write_files
...
def determine_fragment_indices(fragment_1_to_2, fragment_2_to_1, covpenergies, n_mo, idx_homo):
    ...
    n_occ_1, n_occ_2, n_virt_1, n_virt_2 = get_n_occ_virt_per_fragment(
        orbital_indices[0], orbital_indices[1], orbital_indices[2], orbital_indices[3], n_orb_t
    )
...
def main(args):
    ...
    # Determine the actual orbital indices each COVP corresponds to.
    fragment_indices = determine_fragment_indices(
        fragment_1_to_2, fragment_2_to_1, covpenergies, n_mo, idx_homo
    )
    ...
    fragment_1_to_2_net = {"de_alph": 0.0, "de_alph_pct": 0.0, "dq_alph": 0.0, "dq_alph_pct": 0.0}
    fragment_2_to_1_net = {"de_alph": 0.0, "de_alph_pct": 0.0, "dq_alph": 0.0, "dq_alph_pct": 0.0}
    ...
    print(
        format_string_net.format("", fragment_2_to_1_net["de_alph"], fragment_2_to_1_net["dq_alph"])
    )
    print("Net CT into Fragment 2:")
    print(
        format_string_net.format("", fragment_1_to_2_net["de_alph"], fragment_1_to_2_net["dq_alph"])
    )
    ...
    if args["--del"]:
        ...
        for entry in fragment_entries:
            newlen = max(len(str(entry["orb_occ"])), len(str(entry["orb_virt"])))
            if newlen > maxlen:
                maxlen = newlen
        template = "mo.{}.cube"
        for entry in fragment_entries:
