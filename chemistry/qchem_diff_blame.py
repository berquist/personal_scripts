...
def blame_tree(dir1, dir2):
    ...
    for common_file in dcmp.common_files:
        ...
        try:
            blame1 = (
                sp.check_output(
                    " ".join(["git", "blame", os.path.join(dir1, common_file)]), shell=True
                )
                .decode()
                .splitlines()
            )
        ...
        try:
            blame2 = (
                sp.check_output(
                    " ".join(["git", "blame", os.path.join(dir2, common_file)]), shell=True
                )
                .decode()
                .splitlines()
            )
    ...
    for common_dir in dcmp.common_dirs:
        blame_tree(os.path.join(dir1, common_dir), os.path.join(dir2, common_dir))
